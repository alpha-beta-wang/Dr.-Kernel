...
def get_wandb_resume_info(default_local_dir):
    """
    Get wandb resume id from environment variable or local file
    """
    resume_id = None
    resume_mode = None
    fork_from = None
    run_id_file = os.path.join(default_local_dir, "wandb_run_id.txt")

    if os.path.exists(run_id_file):
        # There is already an id file
        # This can only happen when a previous run on this machine crashes
        # Resume from this id with high priority
        with open(run_id_file) as f:
            resume_id = f.read().strip()
        resume_mode = "must"
        print(f"wandb resuming from crashed run with id: {resume_id} from {default_local_dir}")
    elif os.environ.get("WANDB_FORK_STEP"):
        # Fork previous wandb run at certain step
        # wandb resume id can be get in the final part of https://wandb.ai/{user_name}/{project_name}/runs/{id}
        ckpt_step = os.environ.get("WANDB_FORK_STEP")
        old_id = os.environ.get("WANDB_RESUME_ID")
        fork_from = f"{old_id}?_step={ckpt_step}"
        print(f"wandb forking from {fork_from}")
    elif os.environ.get("WANDB_RESUME_ID"):
        # manually resuming from a previous run
        resume_id = os.environ.get("WANDB_RESUME_ID")
        resume_mode = "must"
        print(f"wandb resuming from previous id: {resume_id}")
    else:
        print(f"wandb logging from scratch")
    return resume_id, resume_mode, fork_from
    ...
    def __init__(self, project_name, experiment_name, default_backend: Union[str, List[str]] = 'console', config=None):
        ...
        if 'tracking' in default_backend or 'wandb' in default_backend:
            ...
            wandb.init(
                ...
                resume=resume_mode,
                id=wandb_resume_id,
                # (TODO) Qian: temporarily disable fork_from, since it is not supported by the byted-wandb
                # fork_from=fork_from,
            )
            self.logger['wandb'] = wandb

            os.makedirs(default_local_dir, exist_ok=True)
            with open(os.path.join(default_local_dir, "wandb_run_id.txt"), "w") as f:
